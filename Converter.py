...
# Function to format datetime object to string
...
# Initialise the satellites list
satellites = []
satellites = tle_data
...
# Initialise empty strings to store paragraphs
strs1 = ["" for x in range(len(data)+1)]
strs2 = ["" for x in range(len(data)+1)]

# Define units for the extracted data
...
# Append summary paragraph to the list
...
# Store paragraphs in the list
strs1[k+1] = paragraph
strs2[k+1] = "Summary"

# Create sequence for DataFrame
seq = list(range(len(strs1)))

# Create DataFrame with paragraphs
...
# Set properties for the indexer
...
# Store "text" column in a list
data1 = []
for row in df1.itertuples():
    data1.append( row.text)
# Initialise IterDictIndexer
